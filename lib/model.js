...
let _populateHelpers = null;
function getPopulateHelpers() {
  if (!_populateHelpers) {
    _populateHelpers = {
      assignVals: require('./helpers/populate/assignVals'),
      createPopulateQueryFilter: require('./helpers/populate/createPopulateQueryFilter'),
      getModelsMapForPopulate: require('./helpers/populate/getModelsMapForPopulate'),
      leanPopulateMap: require('./helpers/populate/leanPopulateMap'),
      removeDeselectedForeignField: require('./helpers/populate/removeDeselectedForeignField'),
      splitPopulateQuery: require('./helpers/populate/splitPopulateQuery')
    };
  }
  return _populateHelpers;
}
...
async function _populatePath(model, docs, populateOptions) {
  ...
  const populateHelpers = getPopulateHelpers();
  const modelsMap = populateHelpers.getModelsMapForPopulate(model, docs, populateOptions);
  ...
  for (let i = 0; i < len; ++i) {
    ...
    const splitParams = populateHelpers.splitPopulateQuery(mod, ids, select, assignmentOpts);
    if (splitParams != null) {
      params.push(...splitParams);
      continue;
    }

    const match = populateHelpers.createPopulateQueryFilter(ids, mod.match, mod.foreignField, mod.model, mod.options.skipInvalidIds);
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    populateHelpers.removeDeselectedForeignField(mod.foreignField, mod.options, mod._assignFromOwnResults ? valsByParam[i] : vals);
  }
  for (let i = 0; i < params.length; i++) {
    const mod = params[i][0];
  ...
  return query.exec().then(
    docs => {
      for (const val of docs) {
        getPopulateHelpers().leanPopulateMap.set(val, mod.model);
      }
