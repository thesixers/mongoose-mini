...
const Kareem = require('./helpers/kareem');
...
const mpath = require('./helpers/mpath');
...
function Schema(obj, options) {
  ...
  // Unified path blueprint: single Map combining paths, subpaths, singleNestedPaths, and nested.
  // Values are SchemaType instances, except 'nested' entries which are the string 'nested'.
  // This enables O(1) lookup in schema.path() replacing 3 sequential Object.hasOwn() fallbacks.
...
Schema.prototype._clone = function _clone(Constructor) {
  ...
  // Rebuild _pathMap from cloned data structures to keep blueprint in sync
  s._pathMap = new Map();
  for (const [key, value] of Object.entries(s.paths)) {
    s._pathMap.set(key, value);
  }
  for (const [key, value] of Object.entries(s.subpaths)) {
    s._pathMap.set(key, value);
  }
  for (const [key, value] of Object.entries(s.nested)) {
    if (!s._pathMap.has(key)) {
      s._pathMap.set(key, 'nested');
    }
  }
  for (const schemaType of Object.values(s.paths)) {
    if (schemaType.$isSingleNested) {
      ...
      for (const key of Object.keys(schemaType.schema.paths)) {
        const _fullKey = path + '.' + key;
        s.singleNestedPaths[_fullKey] = schemaType.schema.paths[key];
        s._pathMap.set(_fullKey, schemaType.schema.paths[key]);
      }
      for (const key of Object.keys(schemaType.schema.singleNestedPaths)) {
        const _fullKey = path + '.' + key;
        s.singleNestedPaths[_fullKey] =
          schemaType.schema.singleNestedPaths[key];
        s._pathMap.set(_fullKey, schemaType.schema.singleNestedPaths[key]);
      }
      for (const key of Object.keys(schemaType.schema.subpaths)) {
        const _fullKey = path + '.' + key;
        s.singleNestedPaths[_fullKey] =
          schemaType.schema.subpaths[key];
        s._pathMap.set(_fullKey, schemaType.schema.subpaths[key]);
      }
      for (const key of Object.keys(schemaType.schema.nested)) {
        const _fullKey = path + '.' + key;
        s.singleNestedPaths[_fullKey] = 'nested';
        if (!s._pathMap.has(_fullKey)) {
          s._pathMap.set(_fullKey, 'nested');
        }
      }
    }
  }
...
Schema.prototype.path = function(path, obj) {
  if (obj === undefined) {
    // Fast path: single O(1) Map lookup covers paths, subpaths, singleNestedPaths, and nested.
    const blueprintEntry = this._pathMap.get(path);
    if (blueprintEntry !== undefined) {
      // 'nested' sentinel means it is a container, not a leaf SchemaType
      return blueprintEntry === 'nested' ? undefined : blueprintEntry;
    }

    if (Object.hasOwn(this.paths, path)) {
      this._pathMap.set(path, this.paths[path]);
      return this.paths[path];
    }

    // Positional path fallback (e.g. 'arr.0.name' -> 'arr.$.name')  re: gh-6405
    const cleanPath = _pathToPositionalSyntax(path);
    if (cleanPath !== path) {
      const cleanEntry = this._pathMap.get(cleanPath);
      if (cleanEntry !== undefined) {
        return cleanEntry === 'nested' ? undefined : cleanEntry;
      }
      if (Object.hasOwn(this.paths, cleanPath)) {
        this._pathMap.set(cleanPath, this.paths[cleanPath]);
        return this.paths[cleanPath];
      }
    }
    ...
    const schematype = this.hasMixedParent(cleanPath);
    if (schematype != null) {
      return schematype;
    }
  ...
  for (const sub of subpaths) {
    ...
    if (!branch[sub]) {
      this.nested[fullPath] = true;
      this._pathMap.set(fullPath, 'nested'); // mirror nested container into unified blueprint
      branch[sub] = {};
    }
  ...
  this._pathMap.set(path, schemaType); // mirror into unified blueprint
  ...
  if (schemaType.$isSchemaMap) {
    ...
    this._pathMap.set(mapPath, schemaType.$__schemaType); // mirror into unified blueprint
  ...
  if (schemaType.$isSingleNested) {
    for (const key of Object.keys(schemaType.schema.paths)) {
      const _fullKey = path + '.' + key;
      this.singleNestedPaths[_fullKey] = schemaType.schema.paths[key];
      this._pathMap.set(_fullKey, schemaType.schema.paths[key]); // mirror into unified blueprint
    }
    for (const key of Object.keys(schemaType.schema.singleNestedPaths)) {
      const _fullKey = path + '.' + key;
      this.singleNestedPaths[_fullKey] =
        schemaType.schema.singleNestedPaths[key];
      this._pathMap.set(_fullKey, schemaType.schema.singleNestedPaths[key]); // mirror into unified blueprint
    }
    for (const key of Object.keys(schemaType.schema.subpaths)) {
      const _fullKey = path + '.' + key;
      this.singleNestedPaths[_fullKey] =
        schemaType.schema.subpaths[key];
      this._pathMap.set(_fullKey, schemaType.schema.subpaths[key]); // mirror into unified blueprint
    }
    for (const key of Object.keys(schemaType.schema.nested)) {
      const _fullKey = path + '.' + key;
      this.singleNestedPaths[_fullKey] = 'nested';
      this._pathMap.set(_fullKey, 'nested'); // mirror into unified blueprint
    }
  ...
  if (schemaType.$isMongooseArray && !schemaType.$isMongooseDocumentArray) {
    ...
    while (_schemaType.$isMongooseArray) {
      ...
      this._pathMap.set(arrayPath, _schemaType); // mirror into unified blueprint
    }

    for (const _schemaType of toAdd) {
      this.subpaths[_schemaType.path] = _schemaType;
      this._pathMap.set(_schemaType.path, _schemaType); // mirror into unified blueprint
    }
  }
  ...
  if (schemaType.$isMongooseDocumentArray) {
    for (const key of Object.keys(schemaType.schema.paths)) {
      ...
      const _fullKey = path + '.' + key;
      this.subpaths[_fullKey] = _schemaType;
      this._pathMap.set(_fullKey, _schemaType); // mirror into unified blueprint
    ...
    path.forEach(function(name) {
      ...
      if (this.nested[name]) {
        ...
        for (const path of allKeys) {
          if (path.startsWith(name + '.')) {
            ...
            this._pathMap.delete(path); // keep blueprint in sync
        ...
        this._pathMap.delete(name); // keep blueprint in sync
      ...
      for (const subpath of Object.keys(this.paths)) {
        if (subpath.startsWith(name + '.')) {
          ...
          this._pathMap.delete(subpath); // keep blueprint in sync
        }
      }
      this.mapPaths = this.mapPaths.filter(
        schemaType => !schemaType.path.startsWith(name + '.')
...
Schema.prototype.compileValidator = function compileValidator() {
  if (this._compiledValidator) {
    return this._compiledValidator;
  }
  const self = this;
  this._compiledValidator = function compiledValidator(doc) {
    let hasError = false;
    const errors = {};
    for (const p in self.paths) {
      const st = self.paths[p];
      if (!st || !st.validators || st.validators.length === 0) continue;
      const val = utils.getValue(p, doc);
      for (let i = 0; i < st.validators.length; i++) {
        const v = st.validators[i];
        if (v.validator && !v.validator.call(doc, val, doc)) {
          errors[p] = v.message;
          hasError = true;
          break;
        }
