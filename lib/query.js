...
const mquery = require('./helpers/mquery');
...
/**
 * Sets the `lazy` option for JIT (Just-In-Time) document hydration.
 *
...
Query.prototype.lazy = function(v) {
  this._mongooseOptions.lazy = arguments.length ? v : true;
  return this;
};
...
Query.prototype._completeMany = async function _completeMany(docs, fields, userProvidedFields, opts) {
  const model = this.model;
  const isLazy = this._mongooseOptions.lazy;
  const options = isLazy ? { ...opts, lazy: true } : opts;

  return Promise.all(docs.map(doc => new Promise((resolve, reject) => {
    helpers.createModelAndInit(model, doc, fields, userProvidedFields, options, opts?.populated, (err, resDoc) => {
      if (err != null) {
        return reject(err);
      }
      if (opts?.session != null && resDoc?.$session) {
        resDoc.$session(opts.session);
      }
      resolve(resDoc);
    });
  })));
